...
export default function NewPaymentForm({ approvedVendors, pendingCount }: { approvedVendors: Vendor[]; pendingCount: number }) {
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: 14 }}>
            ...
            {approvedVendors.length === 0 ? (
              <div style={{ fontSize: 12.5, padding: "10px 13px", borderRadius: 9, ...(pendingCount > 0
                ? { color: "#8a6510", background: "#fcf7e6", border: "1px solid #e8d28a" }
                : { color: "#b3261e", background: "#fdeceb", border: "1px solid #f1c5c1" }) }}>
                {pendingCount > 0
                  ? <><strong>{pendingCount} vendor{pendingCount > 1 ? "s" : ""} pending KYB review.</strong> An Owner must approve them in the <Link href="/vendors" style={{ color: "#8a6510", fontWeight: 600, textDecoration: "underline" }}>Vendors tab</Link> before they can receive payments.</>
