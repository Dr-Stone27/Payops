...
| 3 | Live settlement bypasses signed webhook + NIP | **REBUILD (serverless-safe)** | Extract the webhook route's reconciliation into a shared `reconcileSettlement()`; call it **synchronously in-request** from `approvePayment` (NOT background — Vercel kills post-response work, see retrospective §3). Creates `WebhookEvent`, runs NIP, sets reconciled/AMOUNT_MISMATCH/PSP_FAILURE. External HMAC webhook kept as real-PSP seam. Acceptance: real `WebhookEvent` row; `EXC-`→PSP_FAILURE, `MIS-`→AMOUNT_MISMATCH; works on Vercel. Tradeoff: no async "processing" dwell (roadmap). | Honest reconciliation (not fake real-time) | M | 1 |
...
2. **Settlement → REBUILD (real reconciliation), serverless-safe.** ✓ Route approval through the webhook's reconciliation logic (WebhookEvent + NIP + dedup) via a shared function called **synchronously in-request** — NOT background/async dispatch (Vercel kills post-response promises; see `docs/retrospective.md` §3). External HMAC webhook endpoint kept as the real-PSP seam. Real async settlement is roadmap, not MVP.
